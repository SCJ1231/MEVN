...
const main = async()=>{
    const test1 = await Sensor.aggregate(
        [
            {$match : {temp : {$gte : 26}}}, // 조건을 설정합니다.
            {$group : {_id : "$id", total : {$sum : "$temp"}}}
            //그룹화할 유니크한 _id를 고른 후 그룹 후 모든 temp의 합계를 구합니다.
        ]
    )
    console.log(test1)
    //[{ _id: 402, toatal: 31374.8 }]
    
    const test2 = await Sensor.aggregate(
        [
            {$match : {$or : [{temp : {$gte : 26, $lte : 27}}, {humi : 
        {$gte : 80}}]}}, //조건을 설정
            {$group : {_id : "$id", count : {$sum : 1}}},
            {$project : {_id : 0, count : 1}} // return되는 결괏값의 영역을 설정합니다.
    ...
    const test3 = await Sensor.aggregate(
        [
            {$match : {$and : [{temp : {$eq : 26.2}}, {humi : {$eq :
        85.7}}]}}, // 조건을 설정
            {$sort : {temp : 1}},
            {$limit : 5},
            {$project : {_id : 0, temp : 1, time: 1, min: {$minute :
        "$time"}}},
            {$match : {min : {$eq : 1}}}
        ]
    )
